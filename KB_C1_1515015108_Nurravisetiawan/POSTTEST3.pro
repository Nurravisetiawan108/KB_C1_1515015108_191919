...
clauses
	...
	lulus(X,Y):-
		nilaimk(X,Y),Y<>"D",Y<>"E". %mahasiswa lulus jika nilai matakuliah tidak sama dengan D dan tidak sama dengan E
		
	tdklulus(X,Y):-
		nilaimk(X,Y),Y<>"A",Y<>"B",Y<>"C". %mahasiswa tidak lulus jika matakuliah tidak sama dengan A,B, dan C
